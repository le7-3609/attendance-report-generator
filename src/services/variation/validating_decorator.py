"""Validation/audit decorator for variators.
...
class ValidatingVariatorDecorator(BaseVariator):
    """Wraps a variator and audits its output for consistency."""
    ...
    def _audit(self, data: ReportData) -> None:
        ...
        for idx, row in enumerate(data.rows):
            self._audit_row(idx, row, data.report_type)
    ...
    def _audit_row(self, idx: int, row: AttendanceRow, rt: ReportType) -> None:
        if row.date is None:
        ...
        if rt == ReportType.TYPE_B:
            # Net hours subtract break (if present). Treat 00:00 as "no real break".
            break_h = 0.0
        ...
        if abs(row.total_hours - expected_total) > self._max_hours_error:
            raise TransformationError(
                "Validation failed: row[{idx}] total_hours mismatch "
        ...
        if rt == ReportType.TYPE_B:
            ...
            if abs(tier_sum - row.total_hours) > 0.05:
                raise TransformationError(
                    "Validation failed: row[{idx}] overtime tiers do not sum to total "
                    f"(tiers={tier_sum:.2f}, total={row.total_hours:.2f})."
                )

            # Optional but useful: sabbath flag must match the date.
            if row.is_sabbath != (row.date.weekday() == 5):
                raise TransformationError(
    ...
    def _audit_type_b(self, data: ReportData) -> None:
        ...
        expected_total = round(sum(r.total_hours for r in data.rows), 2)
        expected_100 = round(sum(r.hours_100 for r in data.rows), 2)
        expected_125 = round(sum(r.hours_125 for r in data.rows), 2)
        expected_150 = round(sum(r.hours_150 for r in data.rows), 2)
        expected_work_days = sum(1 for r in data.rows if not r.is_sabbath)
